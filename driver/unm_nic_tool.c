...
static int unm_nic_do_ioctl(struct unm_adapter_s *adapter, void *u_data)
{
	...
	switch (data.cmd) {
		...
		case UNM_NIC_CMD_GET_LIC_FINGERPRINT:
			nx_lic_finger_print = kmalloc(sizeof(nx_finger_print_t), GFP_KERNEL);
			if (!nx_lic_finger_print)
				return -ENOMEM;
		...
		case UNM_NIC_CMD_GET_LIC_FEATURES:
			nx_lic_capabilities = kmalloc(sizeof(nx_license_capabilities_t), GFP_KERNEL);
			if (!nx_lic_capabilities)
				return -ENOMEM;
