...
#else
#define NX_PREP_AND_SCHED_RX(NETDEV, SDS_RING) napi_schedule(&((SDS_RING)->napi))     
...
#define DB_NORMALIZE(adapter, off, add) \
	do { \
		BUG_ON(adapter->ahw.db_base == 0UL); \
		add = (void *)(uptr_t)(adapter->ahw.db_base + (off)); \
	} while (0)
...
#else
#define MSIX_ENTRIES_PER_ADAPTER	4
#endif
...
#define UNM_HOST_DUMMY_DMA_SIZE         1024
	...
	int (*macaddr_set) (struct unm_adapter_s *adapter, __uint8_t * addr);	
	int (*change_mtu) (struct net_device *netdev, int mtu);
	void (*set_multi) (struct net_device *netdev);
...
int unm_nic_p2_macaddr_set(struct unm_adapter_s *adapter, __uint8_t * addr);
...
#define NX_NIC_DEV_READY	0
#define NX_NIC_DEV_TX_TIMEOUT	1
#define NX_NIC_DEV_RESET	2
#define NX_NIC_DEV_AER		3
#define NX_NIC_DEV_FAILED	4
