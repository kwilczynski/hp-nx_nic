...
int rom_lock(unm_adapter *adapter);
int rom_unlock(unm_adapter *adapter);
...
static int num_rx_queues = NETQ_MAX_RX_CONTEXTS;
#else
static int multi_ctx            = 0;
static int num_rx_queues = 0;
#endif
...
#ifdef CONFIG_XEN
static int nic_config_bdg(struct unm_adapter_s *adapter, int enable);
#endif
...
static int netxen_nic_change_mtu(struct net_device *netdev, int mtu);
...
static int nx_alloc_adapter_sds_rings(struct unm_adapter_s *adapter)
{
	...
	if (adapter->multi_ctx) {
		adapter->num_sds_rings  = num_rx_queues + 1;
	} else
#endif
	{
...
#ifdef ESX
	if( num_rx_queues < 0 || num_rx_queues > NETQ_MAX_RX_CONTEXTS) {
		num_rx_queues = NETQ_MAX_RX_CONTEXTS;
		nx_nic_print4(NULL, "Invalid value for num_rx_queues, valid value is "
                                "between 0 and %d, setting to default value %d\n", num_rx_queues, num_rx_queues);
	}
#endif
}
...
#ifdef ESX
		mask = DMA_BIT_MASK(64); /* Workaround for KL netdev dma mask assertion. */
#else
		mask = DMA_BIT_MASK(35);
#endif
		cmask =  DMA_BIT_MASK(32);
		adapter->pci_using_dac = 1;
		if(adapter->ahw.revision_id == NX_P2_C0) {
			adapter->dma_mask = DMA_BIT_MASK(32);
			mask = DMA_BIT_MASK(32);
			cmask = DMA_BIT_MASK(32);
			adapter->pci_using_dac = 0;
...
static int nx_enable_msi_x(struct unm_adapter_s *adapter)
{
	...
	nx_nic_print5(NULL, "using MSI-X interrupt\n");
...
static int nx_enable_msi(struct unm_adapter_s *adapter)
{
	...
	if (!pci_enable_msi(adapter->pdev)) {
		...
		nx_nic_print5(NULL, "using MSI interrupt\n");
		return 0;
	}
...
static int nx_start_firmware(struct unm_adapter_s *adapter)
{
	...
	if (adapter->ahw.boardcfg.board_type == UNM_BRDTYPE_P3_HMEZ ||
		...
		rom_lock(adapter);	/* Grab the lock so that no one can read
					...
					/* do all except CAM */
		NXWR32(adapter, UNM_ROMUSB_GLB_SW_RESET, 0xfeffffff);
...
#if (!defined(CONFIG_VMNIX) && !defined (__VMKERNEL_MODULE__) && !defined(NX_FW_LOADER))
        fw_load = LOAD_FW_FROM_FLASH; //Always load firmware from flash for boot ESX driver.
#endif
...
static inline void check_8021q(struct sk_buff *skb, int *vidp)
{
#ifdef NX_VLAN_ACCEL
...
static const struct net_device_ops netxen_netdev_ops = {
	.ndo_open          = unm_nic_open,
	.ndo_stop          = unm_nic_close,
	.ndo_start_xmit    = unm_nic_xmit_frame,
	.ndo_get_stats     = unm_nic_get_stats,
	.ndo_validate_addr = eth_validate_addr,
	.ndo_do_ioctl	   = unm_nic_ioctl,
	.ndo_set_multicast_list = netxen_set_multicast_list,
	.ndo_set_mac_address    = unm_nic_set_mac,
	.ndo_change_mtu    = netxen_nic_change_mtu,
#ifdef NX_VLAN_ACCEL
	.ndo_vlan_rx_register   = nx_vlan_rx_register,
	.ndo_vlan_rx_kill_vid   = nx_rx_kill_vid,
#endif
	.ndo_tx_timeout    = unm_tx_timeout,
#ifdef CONFIG_NET_POLL_CONTROLLER
	.ndo_poll_controller = unm_nic_poll_controller,
#endif
};
#endif

static inline void netxen_set_netdev_ops(struct unm_adapter_s *adapter,
	struct net_device *netdev)
{
...
#endif
	if (NX_IS_REVISION_P2(adapter->ahw.revision_id)) {
		adapter->set_multi = nx_nic_p2_set_multi;
		adapter->change_mtu = nx_nic_fw34_change_mtu;
	} else {
		adapter->set_multi = nx_nic_p3_set_multi;
		adapter->change_mtu = nx_nic_fw40_change_mtu;
	}
}

#if (!defined(ESX) && defined(CONFIG_PCIEAER))
static void unm_mask_aer_correctable(struct unm_adapter_s *adapter)
{
	struct pci_dev *pdev = adapter->ahw.pdev;
	struct pci_dev *root = pdev->bus->self;
	u32 aer_pos;

	if (adapter->ahw.boardcfg.board_type != UNM_BRDTYPE_P3_4_GB_MM &&
	    adapter->ahw.boardcfg.board_type != UNM_BRDTYPE_P3_10G_TP)
		return;
				   ...
				   const struct pci_device_id *ent)
{
	...
	uint8_t *db_ptr = NULL;
	unsigned long long mem_base = 0;
	unsigned long long db_base = 0;
	unsigned long mem_len, db_len, pci_len0;
	...
	/* 128 Meg of memory */
	nx_nic_print6(NULL, "ioremap from %llx a size of %lx\n", mem_base,
		      mem_len);
		...
		adapter->macaddr_set = &unm_nic_p2_macaddr_set;
		if (mem_len == UNM_PCI_128MB_SIZE) {
			...
			if (mem_ptr0 == NULL || mem_ptr1 == NULL || mem_ptr2 == NULL) {
				nx_nic_print3(NULL, "Cannot remap memory aborting.:\n");
				err = -ENOMEM;
				goto err_ret;
			}
		} else {
		...
		adapter->macaddr_set = &unm_nic_p3_macaddr_set;
	...
	nx_nic_print6(NULL, "doorbell ioremap from %llx a size of %lx\n",
		      db_base, db_len);
...
#if (!defined(ESX) && defined(CONFIG_PCIEAER))
	unm_mask_aer_correctable(adapter);
#endif
	...
	NX_INIT_WORK(&adapter->tx_timeout_task, unm_tx_timeout_task, adapter);
	...
	netxen_set_netdev_ops(adapter, netdev);
	...
	if (nx_enable_msi_x(adapter)) {
		if (nx_enable_msi(adapter))
			nx_nic_print5(NULL, "using legacy interrupt\n");
		/*
				 ...
				 nx_host_rx_ctx_t *nxhal_rx_ctx)
{
	...
	for (ring = 0; ring < adapter->max_rds_rings; ring++) {
		...
		if(nxhal_host_rds_ring->os_data) {
		host_rds_ring =
		    (rds_host_ring_t *) nxhal_host_rds_ring->os_data;
		for (i = 0; i < nxhal_host_rds_ring->ring_size; ++i) {
			...
			pci_unmap_single(adapter->pdev, buffer->dma,
...
static inline void unm_nic_clear_stats(struct unm_adapter_s *adapter)
{
...
#ifdef CONFIG_XEN
	if (adapter->ahw.fw_capabilities_1 & NX_FW_CAPABILITY_BDG)
		nic_config_bdg(adapter, 1);
#endif

	adapter->change_mtu(netdev, netdev->mtu);
		...
		struct net_device_stats *stats)
{
	...
	int ring;
#ifdef ESX
	if(ctx_id < 0 || ctx_id > num_rx_queues)
		return -1;
#endif
				 ...
				 nx_host_rx_ctx_t *nxhal_rx_ctx, int ring)
{
       ...
       read_lock(&adapter->adapter_lock);
       DB_NORMALIZE(adapter, UNM_RCV_PRODUCER_OFFSET, addr);
...
int unm_nic_p3_macaddr_set(struct unm_adapter_s *adapter, __uint8_t * addr)
{
	nx_nic_p3_set_multi(adapter->netdev);
	return 0;
}
static void nx_nic_p3_set_multi(struct net_device *netdev)
{
...
static void netxen_set_multicast_list(struct net_device *netdev)
{
	struct unm_adapter_s *adapter = netdev_priv(netdev);
...
static int netxen_nic_change_mtu(struct net_device *netdev, int mtu)
{
	...
	if (adapter->change_mtu)
		rc = adapter->change_mtu(netdev, mtu);
...
int nx_reset_netq_rx_queues( struct net_device *netdev)
{
	...
	adapter->nx_dev->rx_ctxs_state = 0;
	return 0;
}
#else
int nx_reset_netq_rx_queues( struct net_device *netdev)
...
static void nx_set_dev_failed(struct unm_adapter_s *adapter)
{
	NXWR32(adapter, UNM_FW_RESET, NX_NIC_DEV_FAILED);
}

static void nx_set_dev_ready(struct unm_adapter_s *adapter)
{
	api_lock(adapter);

	NXWR32(adapter, UNM_FW_RESET, NX_NIC_DEV_READY);
...
static void nx_clear_aer_state(struct unm_adapter_s *adapter)
{
	...
	if (dev_state == NX_NIC_DEV_AER)
		NXWR32(adapter, UNM_FW_RESET, NX_NIC_DEV_READY);

	api_unlock(adapter);
}

static int nx_dev_request_aer(struct unm_adapter_s *adapter)
{
	u32 dev_state;
	...
	if (adapter->ahw.pci_func != 0 || dev_state != NX_NIC_DEV_READY) {
		api_unlock(adapter);
		return -EIO;
	}

	NXWR32(adapter, UNM_FW_RESET, NX_NIC_DEV_AER);
	api_unlock(adapter);
...
static int nx_dev_request_tx_timeout(struct unm_adapter_s *adapter)
{
	u32 dev_state;
	...
	NXWR32(adapter, UNM_FW_RESET, NX_NIC_DEV_TX_TIMEOUT);
	api_unlock(adapter);

	return 0;
}

static int nx_dev_request_reset(struct unm_adapter_s *adapter)
{
	u32 dev_state;
	...
	NXWR32(adapter, UNM_FW_RESET, NX_NIC_DEV_RESET);
	api_unlock(adapter);
...
static void unm_watchdog_task_fw_reset(TASK_PARAM adapid)
{
	...
	if (fw_reset > NX_NIC_DEV_TX_TIMEOUT) {
		api_unlock(adapter);
		goto out;
	}
	api_unlock(adapter);
...
static void unm_tx_timeout_task(TASK_PARAM adapid)
{
	...
	if(adapter->tx_timeout_count > UNM_FW_RESET_THRESHOLD) {
		nx_dev_request_tx_timeout(adapter);
		goto out;
...
static int nx_nic_fw40_change_mtu(struct net_device *netdev, int new_mtu)
{
	...
	if ((NXRD32(adapter, UNM_CRB_I2C0 + 0x1c) == NX_SSID_NC375T) &&
		...
		nx_nic_print3(adapter,
			"WARNING: NC375T supports only the default MTU of 1500 bytes\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < adapter->nx_dev->active_rx_ctxs; i++) {
		...
		nx_host_sds_ring_t      *nxhal_sds_ring)
{
	...
	if (nxhal_sds_ring->parent_ctx->this_id == 0) {
					...
					int csum_status, int index)
{
	...
	pci_unmap_single(adapter->pdev, (buffer)->dma, host_rds_ring->dma_size,
			 PCI_DMA_FROMDEVICE);
				   ...
				   statusDesc_t *desc)
{
	...
	nx_host_rds_ring_t *nxhal_host_rds_ring = NULL;
	rds_host_ring_t *host_rds_ring = NULL;
	tnic_rxbuff_t *tnic_rxbuff;
	int vid = -1;

	desc_ctx = desc->type;

	if (desc->opcode == UNM_MSGTYPE_TNIC_GLOBAL_BUFFERS) {
		tnic_rxbuff = (tnic_rxbuff_t *)desc;
		index = tnic_rxbuff->ref_handle;
		desc_ctx = tnic_rxbuff->ring_type;
		length = tnic_rxbuff->len;
	}
	...
	if (desc->opcode == UNM_MSGTYPE_TNIC_GLOBAL_BUFFERS) {
		...
		if (adapter->nx_sds_msg_cb_tbl[UNM_MSGTYPE_TNIC_GLOBAL_BUFFERS].handler !=
		    nx_status_msg_default_handler) {
			ret = adapter->nx_sds_msg_cb_tbl[UNM_MSGTYPE_TNIC_GLOBAL_BUFFERS].handler(netdev,
			    adapter->nx_sds_msg_cb_tbl[UNM_MSGTYPE_TNIC_GLOBAL_BUFFERS].data, &msg, skb);
			goto packet_done_no_stats;
		} else {
			/*Replace kfree_skb() with BUG(), once lSA rx becomes stable*/
			printk("rx buffer handler should always be set\n");
			kfree_skb(skb);
			goto packet_done_no_stats;
		}
	}
				  ...
				  uint32_t ringid, nx_free_rbufs_t *free_list)
{
	...
	while (!list_empty(&free_list->head)) {

		buffer = list_entry(free_list->head.next,
					struct unm_rx_buffer, link);
		     ...
		     nx_host_sds_ring_t *nxhal_sds_ring, int max)
{
	...
	while (count < max) {
		...
		if (desc->opcode == adapter->msg_type_rx_desc ||
		    ...
		    desc->opcode == UNM_MSGTYPE_TNIC_GLOBAL_BUFFERS) {
...
{
#ifdef HAVE_NET_DEVICE_OPS
	return (netdev->netdev_ops->ndo_do_ioctl == unm_nic_ioctl);
#else
...
int nx_nic_get_device_port(struct net_device *netdev)
{
	...
	if (!nx_nic_is_netxen_device(netdev))
		return (-1);
...
static void nx_nic_detach_func(struct unm_adapter_s *adapter)
{
	struct net_device *netdev = adapter->netdev;

	netif_device_detach(netdev);

	spin_lock(&adapter->lock); /* Protext tx timeout handler*/
	if (netif_running(netdev))
		unm_nic_down(netdev);
...
static int nx_nic_attach_func(struct pci_dev *pdev)
{
	...
	err = nx_start_firmware(adapter);
	if (err) {
		nx_nic_print3(adapter, "failed to start firmware\n");
		return err;
	}

	if (adapter->ahw.fw_capabilities_1 & NX_FW_CAPABILITY_LRO)
		if(unm_init_lro(adapter))
			nx_nic_print3(adapter, "LRO Initialization failed\n");
...
static pci_ers_result_t nx_nic_io_slot_reset(struct pci_dev *pdev)
{
	int err = 0;

	err = nx_nic_attach_func(pdev);

	return err ? PCI_ERS_RESULT_DISCONNECT : PCI_ERS_RESULT_RECOVERED;
}

static void nx_nic_io_resume(struct pci_dev *pdev)
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct unm_adapter_s *adapter = netdev_priv(netdev);

	nx_clear_aer_state(adapter);
	pci_cleanup_aer_uncorrect_error_status(pdev);
}
#endif /* (!defined(ESX) && defined(CONFIG_PCIEAER)) */
...
{
	return nx_nic_attach_func(pdev);
}
...
#else
	struct unm_adapter_s *adapter = (struct unm_adapter_s *)netdev_priv(netdev);
	if(adapter->ahw.db_len == 0 || adapter->ahw.cut_through) {
		return 0;
	} else {
			       ...
			       unsigned long event, void *ptr)
{
	...
	NX_UPDATE_BOND_FEATURES(real_dev, real_dev->master);
 ...
 */
static int nic_config_bdg(struct unm_adapter_s *adapter,
			int enable)
{
	nic_request_t req;
				...
				nic_response_t *resp)
{
	...
	nx_nic_print6(adapter, "module %d speed %d status %d "
				"cable_len %d OUI 0x%02X 0x%02X 0x%02X full_duplex %d\n",
					linkevent->module,
					linkevent->link_speed,
					linkevent->link_status,
					linkevent->cable_len,
					linkevent->cable_OUI & 0xff,
					(linkevent->cable_OUI >> 8) & 0xff,
					(linkevent->cable_OUI >> 16) & 0xff,
					linkevent->full_duplex);

	if (linkevent->module ==
			LINKEVENT_MODULE_TWINAX_UNSUPPORTED_CABLE) {

		nx_nic_print2(adapter, "Unsupported cable, OUI 0x%02X 0x%02X 0x%02X "
				"Length %d\n",
				linkevent->cable_OUI & 0xff,
				(linkevent->cable_OUI >> 8) & 0xff,
				(linkevent->cable_OUI >> 16) & 0xff,
