...
unm_nic_get_tx_csum(struct net_device *netdev)
{
	...
	if (NX_IS_REVISION_P2(adapter->ahw.revision_id)) {
		return (netdev->features & NETIF_F_IP_CSUM);
	} else {
...
unm_nic_set_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	...
	if (!rv) {
		...
		if (rv == NX_RCODE_NOT_SUPPORTED) {
			if (ecmd->duplex == DUPLEX_HALF) {
				printk(KERN_ERR
					"Speed and autoneg mode settings "
					"supported, half duplex mode "
					"not supported\n");
			}
			return -EOPNOTSUPP;
		} else {
				printk(KERN_ERR
				"Setting speed, duplex or autoneg failed\n");
				return -EIO;
		}
	}
	return rv;
...
static int unm_nic_nway_reset(struct net_device *netdev)
{
	if (netif_running(netdev)) {
		...
		netdev->netdev_ops->ndo_stop(netdev);	// verify
...
static int unm_nic_set_rx_csum(struct net_device *netdev, uint32_t data)
{
	...
	if(netif_running(netdev) && (data == 0))
