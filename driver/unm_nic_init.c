 ...
 *
 * $Id: unm_nic_init.c,v 1.115.8.1 2011/07/02 07:02:54 amit Exp $
 *
 */
					      ...
					      nic_version_t *version)
{
	...
	if (rv) {
		nx_nic_print7(NULL, "FW[%s] load failed\n", fname);
		return rv;
	}
...
int load_fw(struct unm_adapter_s *adapter, int cmp_versions)
{
	...
	if (NX_IS_REVISION_P2(adapter->ahw.revision_id)) {
		...
		if (mn_present & NX_PEG_TUNE_MN_PRESENT) {
			...
			rv = nx_get_file_firmware(adapter, fwtype,
					     &adapter->bios_ver, &file_ver);
			if (!rv) {
				if (compare_fw_version(&file_ver, &adapter->flash_ver) < 0)
                        		flash = 1;
				goto do_load;
                        }
		} else {
