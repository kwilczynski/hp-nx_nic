...
int unm_init_proc_drv_dir(void) {
	...
	NX_SET_MODULE_OWNER(unm_proc_dir_entry->owner , THIS_MODULE);
...
void unm_init_proc_entries(struct unm_adapter_s *adapter) {
	...
	if(adapter->portnum == 0) {
		if(auto_fw_reset_file) {
			...
			NX_SET_MODULE_OWNER(auto_fw_reset_file->owner, THIS_MODULE);
...
#ifdef UNM_NIC_SNMP
	{
		...
		if (snmp_proc) {
			...
			NX_SET_MODULE_OWNER(snmp_proc->owner, THIS_MODULE);
		       ...
		       int *eof, void *data)
{
	...
	if (adapter->nic_net_stats.data && (!adapter->fw_v34) && 
	    adapter->nx_sds_cb_tbl[NX_NIC_CB_LSA].registered == 1) {
		len += sprintf(buf + len, "\nTOE STATISTICS \n");
		len += sprintf(buf + len, "************** \n");
