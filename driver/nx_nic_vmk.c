...
extern uint64_t  MemMap_GetLastValidMPN(void);
...
#ifdef ESX_3X
	uint64_t max_phys_addr = (uint64_t) MemMap_GetLastValidMPN() * PAGE_SIZE;
#else
	uint64_t max_phys_addr = (uint64_t) vmk_GetLastValidMachPage() * PAGE_SIZE;
#endif
...
int nx_nic_netq_get_queue_vector(vmknetddi_queueop_get_queue_vector_args_t *args)
{
	...
	rv = nx_nic_multictx_get_queue_vector(args->netdev, qid);
	if (rv == -1){
		return VMKNETDDI_QUEUEOPS_ERR;
	}
