                                      ...
                                      void *dbell_reflection_vaddr)
{
        ...
        for (ii=0; ii<8; ii++) {
	        writeq(__cpu_to_le64(post_cmd), dbell_vaddr);
                reflection_value = 
                    __le64_to_cpu(readq(dbell_reflection_vaddr));
                if (reflection_value == post_cmd) {
                        return;
                }
        }
        nx_nic_print3(NULL, "%s: Error: Writing 0x%llx. Read back 0x%llx\n", 
...
nx_free_pexq_dbell(struct unm_adapter_s *adapter)
{
	...
	if (pexq->qbuf_paddr != (dma_addr_t)0) {
		pci_unmap_single(adapter->pdev, pexq->qbuf_paddr,
				 NX_PEXQ_DBELL_BUF_SIZE,
				 PCI_DMA_TODEVICE);
	}
	...
	if (pexq->compl_in_paddr != (dma_addr_t)0) {
		pci_unmap_single(adapter->pdev, pexq->compl_in_paddr,
				 PAGE_SIZE,
				 PCI_DMA_FROMDEVICE);
	}
