...
#define NX_VLAN_DEV_INFO(dev) vlan_dev_real_dev(dev)
#else
#define NX_VLAN_DEV_INFO(dev) VLAN_DEV_INFO(dev)->real_dev
#endif
...
#define IPV6_HDR(SKB)  	 (skb_network_header((SKB)))
...
#define NX_UPDATE_BOND_FEATURES(DEV, MASTER)                                    \
	do {                                                                    \
		if(((DEV)->priv_flags & IFF_BONDING) && (MASTER)) {             \
			(MASTER)->vlan_features =                               \
			netdev_increment_features((MASTER)->vlan_features,      \
					(DEV)->vlan_features,                   \
					NETIF_F_ONE_FOR_ALL | NETIF_F_ALL_CSUM);\
		}                                                               \
	} while (0)
...
#define NX_UPDATE_BOND_FEATURES(DEV, MASTER)                                    \
	do {                                                                    \
		...
		if(((DEV)->priv_flags & IFF_BONDING) && (MASTER)) {             \
			features = (MASTER)->vlan_features;                     \
			features |= (NETIF_F_SG | NETIF_F_FRAGLIST |            \
					NETIF_F_HIGHDMA | NETIF_F_GSO_MASK |    \
					NETIF_F_NO_CSUM);                       \
			(MASTER)->vlan_features |=                              \
			netdev_compute_features(features, (DEV)->vlan_features);\
		}                                                               \
	} while (0)
